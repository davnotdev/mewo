...
impl TermQuad {
    pub fn create(position: TermVector2, qtype: TermQuadType, bg_color: Option<u16>, fg_color: Option<u16>) -> TermQuad {
        TermQuad {
            bg_color: if let Some(c) = bg_color { c } else { TB_DEFAULT },
            fg_color: if let Some(c) = fg_color { c } else { TB_DEFAULT },
            qtype,
            position,
        } 
    }
...
pub fn term_render(_args: &mut SystemArgs, wish: Wish<&TermQuad, ()>) {
    unsafe { tb_clear() };
